...
#define ALL_PERM  PTE_P | PTE_W | PTE_U
...
x86_64_pagetable* allocate_page() {
    for (int pa = 0; pa < NPAGES; pa++){
        if (pageinfo[pa].owner == PO_FREE){
            pageinfo[pa].owner = g_owner;
            pageinfo[pa].refcount = 1;
            x86_64_pagetable* page = PAGEADDRESS(pa);
...
x86_64_pagetable* copy_pagetable(x86_64_pagetable* pagetable, int8_t owner) {
    g_owner = owner;
    x86_64_pagetable* new_pagetable = allocate_page();
    // memset(new_pagetable, 0, sizeof(new_pagetable));

    for (uintptr_t va = 0; va < PROC_START_ADDR; va += PAGESIZE) {
        vamapping vam = virtual_memory_lookup(pagetable, va);
        virtual_memory_map(new_pagetable, va, vam.pa, PAGESIZE, vam.perm, allocate_page);
    }
...
void kernel(const char* command) {
    ...
    virtual_memory_map(kernel_pagetable, 0x0, 0x0,
                    PROC_START_ADDR, PTE_P | PTE_W, allocate_page);
    virtual_memory_map(kernel_pagetable, 0xB8000, 0xB8000,
                    PAGESIZE, ALL_PERM, allocate_page);
...
void process_setup(pid_t pid, int program_number) {
    ...
    int r = program_load(&processes[pid], program_number, allocate_page);
    assert(r >= 0);

    processes[pid].p_registers.reg_rsp = MEMSIZE_VIRTUAL;
    uintptr_t stack_page = processes[pid].p_registers.reg_rsp - PAGESIZE;
    uintptr_t next_free = allocate_page();
    
    virtual_memory_map(processes[pid].p_pagetable, stack_page, next_free, PAGESIZE, ALL_PERM, allocate_page);
...
void exception(x86_64_registers* reg) {
    ...
    case INT_SYS_FORK: {
        ...
        for (uintptr_t va = PROC_START_ADDR; va < MEMSIZE_VIRTUAL; va += PAGESIZE) {
            ...
            int p = vam.perm & PTE_W;
            if (p) {
                uintptr_t free_page = allocate_page();
                memcpy(free_page, vam.pa, PAGESIZE);
                assign_physical_page(free_page, child_pid);
                virtual_memory_map(processes[child_pid].p_pagetable, va, free_page, PAGESIZE, ALL_PERM, allocate_page);
            }
        }
        memcpy(&(processes[child_pid].p_registers), &(current->p_registers), sizeof(x86_64_registers));
    ...
    case INT_SYS_PAGE_ALLOC: {
        ...
        int map_res = virtual_memory_map(current->p_pagetable, current->p_registers.reg_rdi, next_free, PAGESIZE, ALL_PERM, allocate_page);
        if (map_res < 0) {
            current->p_registers.reg_rax = -1;
        } else {
